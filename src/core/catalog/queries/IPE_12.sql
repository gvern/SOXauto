...
WHERE soi.IS_PREPAYMENT = 0 
    ...
    AND soi.DELIVERED_DATE BETWEEN '2019-01-01 00:00:00' 
        AND CONVERT(DATE, DATEADD(d, -(DAY(DATEADD(m, -1, GETDATE() - 2))), DATEADD(m, 0, GETDATE()-1)))
    AND (troubleshoot_resolution_date IS NULL 
        OR troubleshoot_resolution_date > CONVERT(DATE, DATEADD(d, -(DAY(DATEADD(m, -1, GETDATE() - 2))), DATEADD(m, 0, GETDATE()-1))))