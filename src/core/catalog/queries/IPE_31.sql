...
SELECT a.*,sn.SERVICE_PROVIDER,cp.Type,cp.ERP_Name,comp.[Company_Country],DATEADD(second, -1, CAST('{subsequent_month_start}' AS DATETIME)) AS Closing_date,
...
AND (CASE
        ...
        THEN ISNULL(p1.payment_reversal_date, t1.created_date)
        ELSE t1.Created_Date
    END) < CAST('{subsequent_month_start}' AS DATETIME)
...
LEFT JOIN (
    ...
    AND (OMS_Close_Date < CAST('{subsequent_month_start}' AS DATETIME) OR OMS_Close_Date IS NULL)
...
LEFT JOIN (
    ...
    FROM [AIG_Nav_Jumia_Reconciliation].[dbo].[RPT_COLLECTIONADJ]
    WHERE OMS_Entity_Type <> 'packlist'
    AND ((OMS_Force_Closed = 1 AND (OMS_Close_Date < CAST('{subsequent_month_start}' AS DATETIME) OR OMS_Close_Date IS NULL)) OR OMS_Close_Date < CAST('{subsequent_month_start}' AS DATETIME))
...
WHERE Company_Country NOT IN {excluded_countries_ipe31}
