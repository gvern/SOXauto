...
# Bridge to unified catalog (read-only helper)
try:
    from src.core.ipe_catalog_pg1 import list_items as list_catalog_items, get_item_by_id as get_catalog_item
except Exception:
    # Soft fallback if catalog module is missing in some environments
    def list_catalog_items(*args, **kwargs):  # type: ignore
        return []

    def get_catalog_item(*args, **kwargs):  # type: ignore
        return None
