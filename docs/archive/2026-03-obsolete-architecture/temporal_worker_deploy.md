...
> ⚠️ Legacy runbook: this document describes the previous Temporal-based orchestration model.
> The project now standardizes orchestration on Apache Airflow.
> For active deployment guidance, use `docs/deployment/airflow_deploy.md`.

This guide documents the **legacy production deployment architecture** that used Temporal + Teleport. It is retained for historical traceability.
